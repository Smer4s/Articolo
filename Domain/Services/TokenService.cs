...
{
	public ApiTokenModel GenerateTokens(User user, JwtSecurityToken? oldToken = default)
	{
		var identity = GetIdentity(user);
		var accessToken = GenerateAccessToken(identity, oldToken);
	...
	public ApiTokenModel RefreshTokens(User user, string refreshToken, string accessToken)
	{
		ValidateRefreshToken(user.RefreshToken, refreshToken);

		return GenerateTokens(user, new JwtSecurityToken(accessToken));
	}
	...
	private string GenerateAccessToken(ClaimsIdentity identity, JwtSecurityToken? oldToken = default)
	{
		var jwt = new JwtSecurityToken(
			claims: identity.Claims,
			expires: DateTime.Now.AddMinutes(AuthOptions.LIFETIME),
			issuer: AuthOptions.ISSUER,
			audience: oldToken is null ? AuthOptions.AUDIENCE : oldToken.Audiences.First(),
